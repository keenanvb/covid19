...
const Compare = ({ data: { selectedCountry, selectedCountryCompare,
    ...
    return (
        <div className="container">
            ...
            <Dropdown dropdownData={countries.filter((country) => {
                return country.title !== selectedCountryDisplay
            })} type='compare' />
            <div className="compare-display">
                <h3>{selectedCountryDisplay == '' ? 'Country to be selected' : selectedCountryDisplay}  vs {selectedCountryCompareDisplay == '' ? 'Country to be selected' : selectedCountryCompareDisplay}</h3>
            </div>
                    ...
                    <>
                            ...
                            {filterList.map((filter, index) => {
                                if (filter === selectSort) {
                                    return (
                                        ...
                                        }} className={`btn btn-light activeFilter`}>{filter.toLowerCase()}</div>
                                    )
                                } else {
                                    return (
                                        <div onClick={() => {
                                            setSelectSort(`${filter}`);
                                        }} className={`btn btn-light`}>{filter.toLowerCase()}</div>
                                    )
                                }
                            })}
                        </div>
                        <ResponsiveContainer width="100%" height={timeseriesHeight}>
                            <AreaChart margin={{ top: 10, right: 10, left: 0, bottom: 10 }} data={countrylist}>
                                ...
                                <Area type="monotone" dataKey={selectedCountryDisplay} stroke="#2ecc71" fillOpacity={1} fill='url(#country1)' />
                                <Area type="monotone" dataKey={selectedCountryCompareDisplay} stroke="#e67e22" fillOpacity={1} fill="url(#country2)" />
