			...
			<Control position="bottomleft">
				<div className="info top5">
					<div style={{ display: 'flex', justifyItems: 'spaceBewteen' }}>
						<button
							className={`${stepTop5 == 3 ? 'arrow' : ''}`}
							disabled={stepTop5 == 1 ? true : false}
							onClick={() => {
						...
						<button
							className={`${stepTop5 == 1 ? 'arrow' : ''}`}
							disabled={stepTop5 == 4 ? true : false}
							onClick={() => {
					...
					<>
						{mapData.length > 0 ? (
							<Map ref={mapRef} center={[-22.93, 30.55]} zoom={3} animate={true}>
								...
								{activeCountry && (
									<Popup
										position={[activeCountry.lat, activeCountry.long]}
										onClose={() => {
									...
									>
										<div>
											...
											<p className="">
												last update:{' '}
												{moment(activeCountry.lastUpdate).format('DD/MM/YY hh:mm:ss a')}
											</p>
											<p className="confirmed">
												Confirmed: {new Intl.NumberFormat('en').format(activeCountry.confirmed)}
											</p>
											<p className="active">
												Active: {new Intl.NumberFormat('en').format(activeCountry.active)}
											</p>
											<p className="recovered">
												Recovered: {new Intl.NumberFormat('en').format(activeCountry.recovered)}
											</p>
											<p className="deaths">
												Deaths: {new Intl.NumberFormat('en').format(activeCountry.deaths)}
											</p>
